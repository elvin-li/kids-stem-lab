...
const MUTATIONS = [
  ...
  },
  {
    ...
    page: 'games/doodle-pad.html',
    /* 计数器取全页各个带标签列表的最大值，而这页有两个会被数到的列表：
       「给家长的问题」h3 后面 6 条，以及「陪伴追问：」b 标签 3500 字符窗口内
       扫到的变量清单 4 条。原来只删一条镜像问题就够（当年列表只有 3 条），
       后来问题扩到 6 条、锚文本也加了解说尾巴，单删一条早已压不到 3 以下。
       现在两个列表都削到 2 条，最大值 = 2，才能证明「少于 3 条会报警」。 */
    mutate: (html) => html
      .replace('        <li>“镜像打开后，哪里让你吃惊？”吃惊的地方就是变换被看见了。</li>\n', '')
      .replace('        <li>“这幅画要不要打分？我们记下什么才算完成？”答案是颜色、笔数、看见了什么，不是分数。</li>\n', '')
      .replace('        <li>“你说差很多——哪一处最不一样？我们只改那一处，好吗？”一次一处，记录才站得住。</li>\n', '')
      .replace('        <li>“看 3&nbsp;秒和看 30&nbsp;秒，多看见了哪一条线？”差在观察时间，不在手笨。</li>\n', '')
      .replace('        <li><b>要观察的：</b>多看到了哪一处，哪一笔更像自己看见的，不是像不像范画。</li>\n', '')
      .replace('        <li><b>另开一行：</b>自评卡选「差很多」时，只改那一处，不整张重画。</li>\n', ''),
    expect: '只有 2 条'
  }
];
